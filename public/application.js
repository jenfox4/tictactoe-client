...
var findPosition = function findPosition(gameBoard, gamePiece) {
  //  gameBoard = game board you are playing, gamePiece = X or O's
  var position = [];
  // array of index position of X or O
  var idx = gameBoard.indexOf(gamePiece);
  while (idx !== -1) {
...
var play = function play(player, positionToPutPiece, gameBoard) {
...
var minimumPlays = function minimumPlays(gameBoard) {
  // checks to see if enough moves have been played for a win
  var xPosition = findPosition(gameBoard, 'x');
  // find all the positions of X
  var oPosition = findPosition(gameBoard, 'o');
...
var checkWinningCondition = function checkWinningCondition(arr) {
...
var computer = function computer(array) {
  var emptySpace = [];
...
var checkWinning = function checkWinning(player) {
...
var pastGames = function pastGames(data) {
  return $.ajax({
    url: config.apiUrl + '/games',
    method: 'GET',
    headers: {
...
var showIndexGame = function showIndexGame() {
  return $.ajax({
...
var store = __webpack_require__(1);
var gamelogicfunctions = __webpack_require__(3);
var config = __webpack_require__(2);

// places x or o in div
var placeXOrO = function placeXOrO(id, player) {
  if (player === 'x') {
...
var switchPlayer = function switchPlayer(str) {
  if (str === 'x') {
...
var winStatus = function winStatus(over, winner) {
  if (over) {
    $('.score').show();
    if (winner === 'x') {
...
var computerGame = function computerGame() {
  $('#message').html('');
};
...
var refresh = function refresh(response) {
  store.game = response.game;
  $('.box').empty();
  $('.row').show();
  $('#message').html('your turn <b>Player One</b>');
...
var newGameFail = function newGameFail(response) {
...
var updateGameFail = function updateGameFail(response) {
...
var pastGames = function pastGames(response) {
  ...
  for (var _i = 0; _i < game.length; _i++) {
    if (gamelogicfunctions.minimumPlays(game[_i].cells) === 'o') {
      lost += 1;
    }
  }
...
var failGames = function failGames() {
  $('#wins').text('Something went wrong, please try again');
};

var computerOptions = function computerOptions() {
  $('.comp-human').hide();
...
var changePlayerIcon = function changePlayerIcon(imageToChange, player) {
...
module.exports = {
  ...
  startGame: startGame,
  changePlayerIcon: changePlayerIcon
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var gameEvents = __webpack_require__(14);
...
var onSignUp = function onSignUp(event) {
  ...
  // console.log(data)
  api.signUp(data).then(ui.signUpSuccess).catch(ui.fail);
};
...
var onSignIn = function onSignIn(event) {
  ...
  var data = getFormFields(event.target);
  // console.log(data)
  api.signIn(data).then(ui.signInSuccess).catch(ui.fail);
};
...
var logOut = function logOut(event) {
  ...
  var data = getFormFields(event.target);
  // console.log(data)
  api.logOut(data).then(ui.logOutSuccess).catch(ui.logOutFail);
};
...
var changePassword = function changePassword(event) {
  ...
  var data = getFormFields(event.target);
  // console.log(data)
  api.changePassword(data).then(ui.changePasswordSuccess).catch(ui.fail);
};

var onChangePlayerModal = function onChangePlayerModal(event) {
  var iconPlayer = event.target.id;
  $('.list').html('');
  var listOfWonders = '';
  for (var imgName in config.imgUrl) {
    var image = config.imgUrl[imgName];
...
var chooseOpponent = function chooseOpponent() {
  ui.chooseOpponent();
};
...
module.exports = {
  ...
  onChangePlayerModal: onChangePlayerModal,
...
var logOutFail = function logOutFail(response) {
...
var store = __webpack_require__(1);
var gamelogicfunctions = __webpack_require__(3);
var computer = __webpack_require__(15);
var config = __webpack_require__(2);
...
var onBoardClick = function onBoardClick(event) {
  ...
  api.updateGame(index, value, store.over).then(ui.placeXOrO(index, player), ui.winStatus(store.over, store.winner)).catch(ui.updateGameFail);
  if (store.over === false && store.opponent === 'human') {
    switchPlayer();
  }
};

var switchPlayer = function switchPlayer(event) {
...
var compOrHuman = function compOrHuman(event) {
...
var createNewGame = function createNewGame(event) {
  store.opponent = event.target.id;
  player = 'x';
  store.over = false;
  store.gameBoard = ['', '', '', '', '', '', '', '', ''];
  api.createNewGame().then(ui.newGameSuccess).catch(ui.newGameFail);
};
...
var refresh = function refresh() {
  ...
  store.over = false;
  store.gameBoard = ['', '', '', '', '', '', '', '', ''];
  api.createNewGame().then(ui.refresh).catch(ui.newGameFail);
};

// calls up past games in APU and has UI respond
var pastGames = function pastGames() {
  api.pastGames().then(ui.pastGames).catch(ui.failGames);
};
...
var choosePlayerIcon = function choosePlayerIcon(event) {
  var hasClass = $(event.target).hasClass('player1');
  var playerIcon = null;
  if (hasClass) {
    playerIcon = 'player1';
  } else {
  ...
  var newIcon = config.imgUrl[playerIcon] = event.target.src;
...
var store = __webpack_require__(1);
var gamelogicfunctions = __webpack_require__(3);
var minimax = __webpack_require__(16);

var player = 'o';
// function that determines game winnings for computer game

var easyComputerPlaying = function easyComputerPlaying() {
  if (store.over || store.invalid) {
    return;
  }
  player = 'o';
  var index = gamelogicfunctions.computer(store.gameBoard);
  // finds id of div player has clicked, same as index in array
  var value = player;
  // finds which player is currently being played
  if (store.gameBoard[index] === '') {
  ...
  api.updateGame(index, value, store.over).then(ui.winStatus(store.over, store.winner)).catch(ui.updateGameFail);
};

var hardComputerPlaying = function hardComputerPlaying() {
  if (store.over || store.invalid) {
    return;
  }
  player = 'o';
  var emptySpaces = minimax.findEmptySpaces(store.gameBoard);
  var index = minimax.bestMoveFinder(minimax.minimax(emptySpaces, store.gameBoard));
  // finds id of div player has clicked, same as index in array
  var value = player;
  // finds which player is currently being played
  store.gameBoard[index] = player;
...
module.exports = {
  easyComputerPlaying: easyComputerPlaying,
  hardComputerPlaying: hardComputerPlaying
};
...
var findEmptySpaces = function findEmptySpaces(gameBoard) {
  var emptySpaces = [];
  var idx = gameBoard.indexOf('');
  while (idx !== -1) {
...
var minimax = function minimax(emptySpaces, gameBoard) {
  var spacesCount = emptySpaces.length;
  for (var i = 0; i < spacesCount; i++) {
    var score = 0;
    var moveNumber = i;
    var newMove = new Move(emptySpaces[0], score);
    moves.push(newMove);
    ...
    var newEmptySpaces = findEmptySpaces(gameBoard);

    for (var _i = 0; _i < newEmptySpaces.length; _i++) {
      gameBoard[newEmptySpaces[_i]] = 'x';
      if (winning(gameBoard, 'x') === true) {
        moves[moveNumber].score -= 10;
      }
      var newNestedEmptySpaces = findEmptySpaces(gameBoard);

      for (var _i2 = 0; _i2 < newNestedEmptySpaces.length; _i2++) {
        gameBoard[newNestedEmptySpaces[_i2]] = 'o';
        if (winning(gameBoard, 'o') === true) {
          moves[moveNumber].score += 10;
        }
        var newNewNestedEmptySpaces = findEmptySpaces(gameBoard);

        for (var _i3 = 0; _i3 < newNewNestedEmptySpaces.length; _i3++) {
          gameBoard[newNewNestedEmptySpaces[_i3]] = 'x';
          if (winning(gameBoard, 'x') === true) {
            moves[moveNumber].score -= 10;
          }
          var yetAnotherEmptySpaces = findEmptySpaces(gameBoard);

          for (var _i4 = 0; _i4 < yetAnotherEmptySpaces.length; _i4++) {
            gameBoard[yetAnotherEmptySpaces[_i4]] = 'o';
            if (winning(gameBoard, 'o') === true) {
              moves[moveNumber].score += 10;
            }
            var andAnotherEmptySpaces = findEmptySpaces(gameBoard);

            for (var _i5 = 0; _i5 < andAnotherEmptySpaces.length; _i5++) {
              gameBoard[andAnotherEmptySpaces[_i5]] = 'x';
              if (winning(gameBoard, 'x') === true) {
                moves[moveNumber].score -= 10;
              }

              var andYesAnotherEmptySpaces = findEmptySpaces(gameBoard);

              for (var _i6 = 0; _i6 < yetAnotherEmptySpaces.length; _i6++) {
                gameBoard[andYesAnotherEmptySpaces[_i6]] = 'o';
                if (winning(gameBoard, 'o') === true) {
                  moves[moveNumber].score += 10;
                }
                var noWayAnother = findEmptySpaces(gameBoard);

                for (var _i7 = 0; _i7 < noWayAnother.length; _i7++) {
                  gameBoard[noWayAnother[_i7]] = 'x';
                  if (winning(gameBoard, 'x') === true) {
                    moves[moveNumber].score -= 10;
                  }
                  gameBoard[noWayAnother[_i7]] = '';
                }
                gameBoard[andYesAnotherEmptySpaces[_i6]] = '';
              }
              gameBoard[andAnotherEmptySpaces[_i5]] = '';
            }
            gameBoard[yetAnotherEmptySpaces[_i4]] = '';
          }
          gameBoard[newNewNestedEmptySpaces[_i3]] = '';
        }
        gameBoard[newNestedEmptySpaces[_i2]] = '';
      }
      gameBoard[newEmptySpaces[_i]] = '';
    }
...
var bestMoveFinder = function bestMoveFinder(moveArr) {
  var bestScore = moveArr[0].score;
  var bestMove = moveArr[0].index;
  for (var i = 1; i < moveArr.length; i++) {
...
var clearMoves = function clearMoves(moveArr) {
...
module.exports = {
  findEmptySpaces: findEmptySpaces,
  minimax: minimax,
